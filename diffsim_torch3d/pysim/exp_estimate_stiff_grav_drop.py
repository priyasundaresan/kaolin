...
ref_verts, ref_faces, _ = load_obj("meshes/rigidcloth/sysid/ref_0.03_9.8.obj")
...
def run_sim(steps, sim):
    stiff_inp, grav_inp = torch.relu(param_g).cpu()

    print(param_g)

    orig = sim.cloths[0].materials[0].stretching
    sim.cloths[0].materials[0].stretching = orig*stiff_inp
    orig_grav = sim.gravity
    sim.gravity = orig_grav*grav_inp
