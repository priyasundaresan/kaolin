...
def get_ref_mesh(sim_step):
    mesh_fnames = sorted([f for f in os.listdir('demo_exp_learn_twoin_raster_vid/out0') if '%04d'%sim_step in f])
    all_verts = []
    all_faces = []
    all_textures = []
    vert_count = 0
    for j, f in enumerate(mesh_fnames[:1]):
        verts, faces, aux = load_obj(os.path.join("demo_exp_learn_twoin_raster_vid", "out0", f))
...
def run_sim(steps, sim, net, epoch):
    ...
    for step in range(steps):
        ...
        #	net_input.append(sim.cloths[0].mesh.nodes[i].x)
        #	net_input.append(sim.cloths[0].mesh.nodes[i].v)
...
with open(out_path+('/log%s.txt'%timestamp),'w',buffering=1) as f:
    ...
    #net = Net(len(sim.cloths[0].mesh.nodes)*6 + 1, 3*len(handles))
