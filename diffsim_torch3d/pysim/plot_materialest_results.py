...
print(results.T)
#ini_p1, ini_p2, _, final_p1, final_p2, _, _, loss = results.T
ini_p1, ini_p2, final_p1, final_p2, _, loss = results.T
...
for i in range(len(final_p1)):
    ...
    sc = ax2.scatter(ini_p1[i], ini_p2[i], color=cmap_loss(norm(loss[i])))
...
ax2.set_xlim(0,np.amax(ini_p1)+(np.amax(ini_p1) - np.amin(ini_p1))/10)
ax2.set_ylim(0,np.amax(ini_p2)+(np.amax(ini_p2) - np.amin(ini_p2))/10)
ax2.set_xlabel("Initial p1")
ax2.set_ylabel("Initial p2")

sm =  ScalarMappable(norm=norm, cmap=cmap_loss)
sm.set_array([])
cbar = fig.colorbar(sm, ax=ax2)
cbar.ax.set_title("# Iters", fontsize=10)
