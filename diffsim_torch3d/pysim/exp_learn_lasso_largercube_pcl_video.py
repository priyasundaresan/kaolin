...
with open('conf/rigidcloth/lasso/start_largercube.json','r') as f:
	config = json.load(f)
...
def get_loss_per_iter(sim, epoch, sim_step):
    ...
    ref_pcl = torch.from_numpy(np.load('demo_exp_learn_lasso_largercube_pcl_video/%03d.npy'%sim_step)).to(device)
