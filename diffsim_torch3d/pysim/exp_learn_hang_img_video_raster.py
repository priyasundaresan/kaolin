...
num_views = 10
num_views_per_iter = 2
elev = torch.linspace(0,360,num_views)
azim = torch.linspace(-180, 180, num_views)
#R, T = look_at_view_transform(2, 0, 30) 
...
def get_ref_mesh(sim_step):
    mesh_fnames = sorted([f for f in os.listdir('demo_exp_learn_hang_img_video_raster/out0') if '%04d'%sim_step in f])
    all_verts = []
    all_faces = []
    all_textures = []
    vert_count = 0
    for j, f in enumerate(mesh_fnames[:1] + mesh_fnames[2:]):
        verts, faces, aux = load_obj(os.path.join("demo_exp_learn_hang_img_video_raster", "out0", f))
...
def get_loss_per_iter(sim, epoch, sim_step, num_points=2000000):
    ...
    #loss = loss_rgb*0.25 + loss_silhouette*0.75 # works well
    loss = loss_rgb*0.5 + loss_silhouette*0.5 # works well
    #loss = loss_rgb*1.0 + loss_silhouette*0.0
    if epoch % 1 == 0:
...
def run_sim(steps, sim, net, epoch):
    ...
    for step in range(steps):
        ...
        for i in range(len(handles)):
            ...
            sim.cloths[0].mesh.nodes[handles[i]].v += net_output
