...
def get_ref_mesh(sim_step):
    mesh_fnames = sorted([f for f in os.listdir('demo_exp_learn_lasso_raster_vid/out0') if '%04d'%sim_step in f])
    all_verts = []
    all_faces = []
    all_textures = []
    vert_count = 0
    for j, f in enumerate(mesh_fnames[:1] + mesh_fnames[2:]):
        verts, faces, aux = load_obj(os.path.join("demo_exp_learn_lasso_raster_vid", "out0", f))
