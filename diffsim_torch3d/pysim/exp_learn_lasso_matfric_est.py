...
with open('conf/rigidcloth/lasso/demo_slow_friction.json','r') as f:
	config = json.load(f)
...
def get_ref_mesh(sim_step):
    mesh_fnames = sorted([f for f in os.listdir('demo_exp_learn_lasso_matfric_est/out0') if '%04d'%sim_step in f])
    all_verts = []
    all_faces = []
    all_textures = []
    vert_count = 0
    for j, f in enumerate(mesh_fnames[:1] + mesh_fnames[2:]):
        verts, faces, aux = load_obj(os.path.join("demo_exp_learn_lasso_matfric_est", "out0", f))
...
def run_sim(steps, sim, epoch):
    ...
    #	node.m *= param[-1].double()

    proportions = F.softmax(param[:-1]).float()
    
    #print("proportions", proportions, "friction", param[-1])
    sim.obs_friction = param[-1]
...
def do_train(cur_step,optimizers,sim):
    ...
    while True:
        ...
        epoch = epoch + 1
        # break
    return F.softmax(param[:-1]), loss, epoch
...
with open(out_path+('/log%s.txt'%timestamp),'w',buffering=1) as f:
    ...
    param = torch.log(initial_probs)
    param = torch.cat((param, torch.tensor([1.0]).double()))
    ...
    #        param_mat = torch.log(initial_probs)
    #        print(initial_probs, F.softmax(param_mat))
    #        param_mat.requires_grad = True
    #        lr = 0.2
    #        optimizer = torch.optim.Adam([param_mat],lr=lr)
